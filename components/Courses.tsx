...
const Courses: React.FC = () => {
  ...
  return (
    <section id="courses" className="py-24 bg-brand-bg relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">
            Choose Your <span className="inline-block bg-brand-blue px-4 py-1 rounded-full text-white transform rotate-1">Path</span>
          </h2>
          <p className="text-gray-600">Find the perfect course to start your Amharic journey. Tap a card for details.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {COURSES.map((course, idx) => {
            ...
            >
              ...
              <div 
                className="absolute top-4 right-4 w-32 h-32 opacity-20 group-hover:opacity-40 transition-opacity bg-contain bg-no-repeat bg-right-top pointer-events-none"
                style={{ backgroundImage: `url(${bgImage})` }}
              ></div>
              <div className="flex justify-between items-start mb-6">
                <div className={`w-14 h-14 rounded-full border border-black flex items-center justify-center text-xl font-bold font-ethiopic ${idx % 2 === 0 ? 'bg-brand-pink text-white' : 'bg-brand-lime text-black'}`}>
              ...
              <h3 className="text-2xl font-bold text-black mb-3 group-hover:text-brand-blue transition-colors">{course.title}</h3>
              <p className="text-gray-600 mb-8 min-h-[48px]">{course.description}</p>
        ...
        {selectedCourse && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="absolute top-0 left-0 w-full h-32 bg-white/50 border-b border-black/5 -z-0"></div>
              <div 
              ...
              <div className="p-8 md:p-10">
                ...
                <h3 className="text-3xl md:text-4xl font-bold mb-4">{selectedCourse.title}</h3>
